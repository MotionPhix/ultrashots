...
onMounted(() => {
  addresses.value = props.modelValue && Array.isArray(props.modelValue)
    ? [...props.modelValue]
    : [{
      ...
      type: props.modelValue?.type ?? 'office'
    }]

  emit('update:modelValue', addresses.value);
})

// Watch for external updates to props.modelValue
...
function onAddressAdd(addressType: string) {
  const newAddress = {street: '', city: '', state: '', country: '', type: addressType};
  addresses.value = [...addresses.value, newAddress];
  emit('update:modelValue', addresses.value);
}

function onAddressRemove(index: number) {
  addresses.value = addresses.value.filter((_, i) => i !== index);
  emit('update:modelValue', addresses.value);
}
</script>
    ...
    <section class="grid grid-cols-2 gap-4 md:gap-6">
      <div class="col-span-2 md:col-span-1">
        <UltraInput
          ...
          rounded-size="md"
          block/>
        <InputError :message="$page.props.errors[`company.address.${idx}.street`]"/>
      </div>
      ...
      <div class="col-span-2 md:col-span-1">
        <UltraInput
          ...
          rounded-size="md"
          block/>
        <InputError :message="$page.props.errors[`company.address.${idx}.city`]"/>
      </div>
      ...
      <div>
        <UltraInput
          ...
          rounded-size="md"
          block/>
        <InputError :message="$page.props.errors[`company.address.${idx}.state`]"/>
      </div>
      ...
      <div>
        <UltraInput
          ...
          rounded-size="md"
          block/>
        <InputError :message="$page.props.errors[`company.address.country`]"/>
      </div>
    </section>

    <InputError :message="$page.props.errors[`company.address.type`]"/>

    <button
      v-if="addresses.length > 1" type="button"
      ...
      <svg
        ...
        class="w-5 h-5 stroke-current" viewBox="0 0 24 24"
